...
<div style="border: 1px solid #000; padding: 10px; margin: 10px; height: 640px; position: relatiive;">
	...
	<table>
		...
		<td><b>Moyenne</b></td>
		<td colspan="4"></td>
		{{ $moyenne = round((float)$totalcomp / $totalcoef, 2)}}
	...
	<div style="display: inline-block; vertical-align:top; width: 45%; margin-right: 5%;">
		...
		<tr>
		    <td>{{ $moyennest1[$eleve->id] }}</td>
		    <td>{{ $moyennest2[$eleve->id] }}</td>
		    <td> {{ $moyenne }}</td>
		    {{-- <td><b>{{ round((float) (($moyennest1[$eleve->id] + $moyennest2[$eleve->id] + $moyenne) / 3) , 2) }}</b></td> --}}
		    <td><b>{{ $eleve->moyenne }}</b></td>
		</tr>
	    </table>
	</div>
